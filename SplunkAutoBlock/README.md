...
A real-time SOC lab that detects brute force attacks and automatically
blocks attacker IPs using Splunk, Python, and PowerShell — with zero
manual intervention.
...
index=* EventCode=4625 | stats count by Source_Network_Address | where
count > 3 | rename Source_Network_Address as src_ip | table src_ip,
count
...
2.  Place scripts in: C:

3.  Enable PowerShell: Set-ExecutionPolicy RemoteSigned
...
Allow Listener Port: New-NetFirewallRule -DisplayName
“SplunkListener8888” -Direction Inbound -Protocol TCP -LocalPort 8888
-Action Allow

Block Attacker IP: New-NetFirewallRule -DisplayName “SPLUNK_BLOCK_”
-Direction Inbound -RemoteAddress -Action Block

Verify Blocked IPs: Get-NetFirewallRule | findstr SPLUNK_BLOCK

Unblock IP: Remove-NetFirewallRule -DisplayName “SPLUNK_BLOCK_”
